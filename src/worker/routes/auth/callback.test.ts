...
class MemoryAuthDatabase implements D1DatabaseWritableLike {
  ...
  prepare(sql: string): D1WritableStatementLike {
    ...
    return {
      ...
      async run() {
        if (sql.includes('INSERT INTO users')) users.add(String(values[0]))
        if (sql.includes('INSERT INTO identities')) {
          ...
          if (index === -1) identities.push(next)
          else identities[index] = next
        }
        return { success: true }
      },
    }
