...
async function fixture() {
  ...
  const budget = vi.fn(async (request: Request) => {
    const body = await request.json() as { type: string }
    if (body.type === 'status') return Response.json({ code: 'status', allowance: { code: 'available', limit: 30, remaining: 29, resetAt: Date.parse('2026-09-11T00:00:00Z'), revision: 1 } })
    return Response.json({ code: body.type === 'begin' ? 'started' : body.type === 'activate' ? 'activated' : body.type === 'reserve' ? 'reserved' : 'settled', operationId: 'operation' })
  })
  ...
  const relay = { deliver: vi.fn(async (_env: unknown, _account: string, _identity: unknown, delivery: { payload: { kind?: string } }) => delivery.payload.kind === 'begin_edit' ? { code: 'begun', show: { id: 'show', name: 'Captured', version: 2 }, context: {} } : { code: 'outcome', receipt: { status: 'applied' } }), query: vi.fn(async () => ({ code: 'outcome', receipt: { status: 'pending' } })) }
...
it('does not claim a builtin connection when the protected provider credential is absent', async () => {
  ...
  const unavailable = { ...f.env, OPENAI_API_KEY: undefined }
  const response = await handleBuiltinHttp(f.request('connect'), unavailable as never, f.relay as never, f.providerFetch)
  expect(await response.json()).toEqual({ code: 'unavailable' })
  expect(await (await handleBuiltinHttp(f.request('status'), unavailable as never, f.relay as never, f.providerFetch)).json()).toEqual({ code: 'unavailable' })
...
it('routes authenticated builtin work through the trusted binding and injected provider only', async () => {
  ...
  expect(await response.json()).toMatchObject({ code: 'outcome', allowance: { code: 'available', remaining: 29 } })
...
it('reads allowance status without claiming a connection or calling the provider', async () => {
  const f = await fixture()
  const response = await handleBuiltinHttp(f.request('status'), f.env as never, f.relay as never, f.providerFetch)
  expect(await response.json()).toEqual({
    code: 'status',
    allowance: { code: 'available', limit: 30, remaining: 29, resetAt: Date.parse('2026-09-11T00:00:00Z'), revision: 1 },
  })
...
it('refuses duplicate activation before relay or provider work', async () => {
  ...
  expect(await (await handleBuiltinHttp(f.request('run'), f.env as never, f.relay as never, f.providerFetch)).json()).toMatchObject({ code: 'duplicate' })
