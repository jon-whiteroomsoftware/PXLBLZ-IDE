...
import { oauthProviderProbe, resetOAuthProviderProbe } from '../../test/oauthProviderFake'
...
afterEach(() => { vi.restoreAllMocks(); resetOAuthProviderProbe() })
...
function harness(initialTime = 1000) {
  ...
  const storage: ConstructorParameters<typeof AgentOAuthAuthority>[0]['storage'] = {
    ...
    async transaction(callback) {
      oauthProviderProbe.transactionDepth++
      try { return await callback(storage) } finally { oauthProviderProbe.transactionDepth-- }
    },
  }
...
it('resolves CIMD before serialized mutation and injects it only for the request', async () => {
  ...
  oauthProviderProbe.client = { clientId, clientName: 'Remote client', redirectUris: ['https://client.test/callback'], tokenEndpointAuthMethod: 'none', grantTypes: ['authorization_code', 'refresh_token'], responseTypes: ['code'] }
  ...
  expect(oauthProviderProbe.lookupDepths).toEqual([0])
...
it('re-resolves current CIMD metadata before token operations', async () => {
  ...
  expect(oauthProviderProbe.lookupDepths).toEqual([0, 0])
})
...
it('revokes an account-bound CIMD grant after its metadata document disappears', async () => {
  ...
  expect(oauthProviderProbe.lookupDepths).toEqual([])
  expect(oauthProviderProbe.revoked).toEqual([[grantId, subject]])
})
