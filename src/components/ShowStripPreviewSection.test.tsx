...
describe('Show strip Preview controls (#968)', () => {
  ...
  it('keeps existing controls live and reports those same values when folded', () => {
    ...
    const brightness = screen.getByRole('slider', { name: 'Show preview brightness' })
    fireEvent.change(brightness, { target: { value: '0.5' } })
    expect(brightness).toHaveAttribute('aria-valuetext', '50%')
    ...
    expect(screen.getByRole('slider', { name: 'Show preview brightness' })).toBeVisible()
