  ...
  return <section aria-label="Replace Group Pattern" className="mt-7 space-y-3">
    ...
    {confirming
      ? <div role="group" aria-label="Confirm Group animation loss" className="space-y-2 rounded-sm border border-amber-800/60 bg-amber-950/30 p-3">
        <p className="text-xs text-amber-200">Replacement drops {losses.length} incompatible control{losses.length === 1 ? '' : 's'}: {losses.map(loss => loss.kind === 'instance-control' ? loss.exportName : loss.kind).join(', ')}. Their animation is removed.</p>
