...
export function ActivityStream({ state, retry, dismiss }: { state: AgentDrawerState; retry: (id: string) => void; dismiss: (id: string) => void }) {
  ...
  return <div className="agent-activity min-h-0 flex-1 overflow-y-auto px-4 py-3" data-testid="agent-chat-log" aria-label="Agent activity">
    ...
    {state.stream.map(line => <div key={line.id} data-testid="agent-chat-line" data-request-id={line.operationId} data-outcome={line.outcome} className={`agent-line my-3 ${failed(line) ? 'text-red-300' : line.kind === 'system' ? 'text-zinc-500' : line.kind === 'author' ? 'text-zinc-400' : 'text-zinc-200'}`}>
      ...
      {failed(line) && !line.dismissed && <div className="ml-3 mt-2 flex flex-wrap items-center gap-2 text-[11px]">
        {state.connection?.kind === 'builtin' && line.retryable && <button type="button" data-testid="agent-chat-retry" data-show-detail-pointer-preserve="true" onPointerDown={event => event.preventDefault()} onClick={() => retry(line.operationId!)} disabled={!!state.request || state.contactLost} className="agent-button">Retry</button>}
        {state.connection?.kind === 'external' && <p className="w-full text-zinc-400">Ask your agent to try again from current state.</p>}
        <button type="button" data-testid="agent-chat-dismiss" data-show-detail-pointer-preserve="true" onPointerDown={event => event.preventDefault()} onClick={() => dismiss(line.operationId!)} className="agent-button">Dismiss</button>
      </div>}
    </div>)}
  </div>
}
