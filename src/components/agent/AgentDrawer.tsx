...
import { agentEdgeState, agentEdgeAccessibleName, type AgentDrawerMode } from '@/engine/agentDrawerModel'
...
function Panel() {
  ...
  const changingDisabled = busy || !!state.request
  return <div className="flex h-full min-h-0 flex-col text-xs" data-testid="agent-chat-panel">
    ...
    {!state.pendingCall && state.connection && <nav className="agent-nav"><button className="text-zinc-400 focus-visible:outline focus-visible:outline-2 focus-visible:outline-agent disabled:cursor-default disabled:opacity-40" type="button" disabled={changingDisabled} onClick={() => controller.changeAgent()}><ChevronLeft aria-hidden size={12} />Change agent</button></nav>}
    {!state.pendingCall && !state.connection && capabilities.external && (state.setupOpen || state.armingUntil !== null) && <nav className="agent-nav"><button className="text-zinc-400 focus-visible:outline focus-visible:outline-2 focus-visible:outline-agent" type="button" onClick={() => controller.backToChooser()}><ChevronLeft aria-hidden size={12} />Back</button></nav>}
    {state.pendingCall ? <AgentCall name={state.pendingCall.name} answer={() => controller.dispatch({ type: 'approveKnock' })} decline={() => controller.dispatch({ type: 'declineKnock' })} /> : state.connection ? <AgentIdentity state={state} disconnect={() => controller.disconnect()} restore={() => controller.restoreContact()} cancel={() => controller.cancel()} /> : capabilities.external && (state.setupOpen || state.armingUntil !== null) ? <AgentArming armed={state.armingUntil !== null} endpoint={capabilities.endpoint} notice={state.setupNotice} ready={() => controller.dispatch({ type: 'connectOwn', now: Date.now() })} cancel={() => controller.dispatch({ type: 'cancelArm' })} /> : <AgentChoose builtin={capabilities.builtin ? () => controller.dispatch({ type: 'chooseBuiltin' }) : undefined} external={capabilities.external ? () => controller.dispatch({ type: 'chooseExternal' }) : undefined} externalBinding={state.externalBinding} moveExternal={() => controller.moveExternal()} movePending={state.movePending || changingDisabled} />}
    <ActivityStream state={state} busy={busy} retry={id => controller.retry(id)} />
    {state.connection?.kind === 'builtin' && <Composer draft={state.draft} change={text => controller.dispatch({ type: 'draft', text })} submit={() => controller.submit()} busy={busy || !!state.request || state.contactLost} allowance={state.allowance} />}
  </div>
}
