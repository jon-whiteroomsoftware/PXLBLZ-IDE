...
it('settles transient push feedback after 3500ms even with the popover closed (#997)', () => {
  vi.useFakeTimers()
  try {
    useControllerStore.setState({ pushResult: { ok: true, created: true } })
    render(<ControllerBar />)
    act(() => { vi.advanceTimersByTime(3499) })
    expect(useControllerStore.getState().pushResult?.ok).toBe(true)
    act(() => { vi.advanceTimersByTime(1) })
    expect(useControllerStore.getState().pushResult).toBeNull()
  } finally { vi.useRealTimers() }
})

it('keeps the keyboard path through the Controller pill, gated reasons, and Escape cancellation (#997)', async () => {
  const user = userEvent.setup()
  setControllerProvider(new ConnectedProvider())
  seedLiveController()
  usePatternStore.setState({ activeDemoName: 'WavyBands' })
  useRouterStore.setState({ route: { kind: 'studio', entity: { kind: 'patterns', id: 'WavyBands' } } })
  useEditorStore.setState({ compileStatus: 'broken', nativeDim: 1, previewSource: 'export function render(index) {}' })
  const pushActivePattern = vi.fn()
  useControllerStore.setState({ pushActivePattern })
  render(<ControllerBar />)
  const pill = screen.getByRole('button', { name: 'Toggle Desk panel' })
  for (let i = 0; i < 20 && document.activeElement !== pill; i++) await user.tab()
  expect(pill).toHaveFocus()
  await user.keyboard('{Enter}')
  const run = screen.getByRole('button', { name: 'Run' })
  const save = screen.getByRole('button', { name: 'Save' })
  for (let i = 0; i < 20 && document.activeElement !== run; i++) await user.tab()
  expect(run).toHaveFocus()
  expectDisabledReason(run, "Fix the pattern's errors before sending")
  await user.keyboard('{Enter}')
  expect(pushActivePattern).not.toHaveBeenCalled()
  await user.tab()
  expect(save).toHaveFocus()
  expectDisabledReason(save, "Fix the pattern's errors before sending")
  act(() => useEditorStore.setState({ compileStatus: 'good' }))
