...
export function ShowStripPreviewSection() {
  ...
  >
    <DeckGrid>
      <DeckSlider label="light size" ariaLabel="Light size" value={lightSize} min={MIN_LIGHT_SIZE} max={MAX_LIGHT_SIZE} step={0.05} onChange={value => { usePreviewStore.getState().setLightSize(value); usePreviewStore.getState().setLightSizeSticky(value) }} />
      <DeckSlider label="diffusion" ariaLabel="Diffusion" value={diffusion} min={0} max={1} step={0.01} presentation="percentage" onChange={value => { usePreviewStore.getState().setDiffusion(value); usePreviewStore.getState().setDiffusionSticky(value) }} />
