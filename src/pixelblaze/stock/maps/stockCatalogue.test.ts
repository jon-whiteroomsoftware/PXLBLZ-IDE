...
describe('source regeneration', () => {
  ...
  it('builds the 490-pixel Eclipse Dome as a spiral-wound dome inside a halo ring', () => {
    const points = mapById('eclipse-dome-2d').resolve(490)
    const dome = points.slice(0, 400)
    const halo = points.slice(400)
    const radius = (point: (typeof points)[number]) => Math.hypot(point.sample[0] - 0.5, point.sample[1] - 0.5)
    const angle = (point: (typeof points)[number]) => Math.atan2(point.sample[1] - 0.5, point.sample[0] - 0.5)

    expect(points).toHaveLength(490)
    ...
    const domeRim = 26.6 / (2 * 47.7)
    expect(Math.max(...dome.map(radius))).toBeLessThanOrEqual(domeRim + 1e-3)
    expect(radius(dome[399])).toBeGreaterThan(domeRim - 0.002)
    for (let i = 1; i < dome.length; i++) expect(radius(dome[i])).toBeGreaterThanOrEqual(radius(dome[i - 1]))

    // The strip winds 12.5 turns clockwise as seen from the front (sample y
    // grows downward). LEDs start half a pitch along the strip, and the apex
    // turns are so short that this half pitch already spans about a third of
    // a turn, so first to last LED covers just over 12.
    let turned = 0
    for (let i = 1; i < dome.length; i++) {
      let step = angle(dome[i]) - angle(dome[i - 1])
      if (step > Math.PI) step -= 2 * Math.PI
      if (step < -Math.PI) step += 2 * Math.PI
      expect(step).toBeGreaterThan(0)
      turned += step
    }
    expect(turned / (2 * Math.PI)).toBeGreaterThan(12)
    expect(turned / (2 * Math.PI)).toBeLessThan(12.5)
  ...
  it('keeps the Eclipse Dome split at other counts', () => {
    const halved = mapById('eclipse-dome-2d').resolve(245)
    expect(halved.slice(0, 200).every((point) => Math.hypot(point.sample[0] - 0.5, point.sample[1] - 0.5) < 0.29)).toBe(true)
    expect(halved.slice(200).every((point) => Math.hypot(point.sample[0] - 0.5, point.sample[1] - 0.5) > 0.49)).toBe(true)
    for (const count of [1, 2, 3, 491]) {
      expect(mapById('eclipse-dome-2d').resolve(count)).toHaveLength(count)
    }
  })
