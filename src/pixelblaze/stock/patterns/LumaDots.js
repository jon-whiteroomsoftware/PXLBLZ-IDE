...
//
// A grayscale dot lattice drifts across the frame on an exact loop. A Show
...
//           loop advances exactly one lattice cell;
//           Direction — Reverse, hold, or forward travel;
//           Spacing — Dot-to-dot distance;
//           Width — Dot size as the lit fraction of each cell;
//           Feather — Edge softness, from hard dots to sine-smooth blooms;
//           Lean — Dot asymmetry, from symmetric through full sawtooth,
...
// perpendicular crest fields, which lights a dot wherever both peak. The
// lattice drifts along the travel heading only, so rows slide as intact
// ranks. Phase accumulates on its own clock, so animating any control from a
// Show never resets position, and one loop advances exactly one period for
