// Pattern: Multisegment Demo
// Built with PXLBLZ-IDE https://pxlblz-ide.whiteroomsoftware.com/
// Credit: "Multisegment Demo" by ZRanger1 - https://patterns.electromage.com/pattern/yfyrMr8XribsGinBf
// License: Redistributed with permission from ZRanger1.
//
// Independent configurable segments turn one strip into several coordinated lighting zones.
// Runs on: 1D strips, rings, and strands.
// Controls: Active Segment — Adjusts active segment; State — Adjusts state; Effect — Adjusts effect; Speed — Adjusts speed; Size — Adjusts size; Color — Adjusts color; Segments — Adjusts segments; Enable Web U I — Adjusts enable web u i.

/*
 Multisegment Pattern Testbed/Demonstrator
 ...
 to allow control from home automation controllers.

 THIS IS THE Web UI ENABLED VERSION, for testing and exploration.
 It will work with home automation controllers, but is NOT RECOMMENDED.

 The automation specific version is available here:
     https://github.com/zranger1/PixelblazePatterns

 (Should you want to use this version with your home automation system,
...
export var __ver = -1       // __ver = -1 tells the HA controller to ignore us
export var __n_segments = 4;  // number of segments (range 1-__max_segments)
export var __boot = 0;     // set to 0 by automation driver after setting segment data
...
// Globals only used by the Web UI
var lastState = 0;
var lastColor = 0;
var lastEffect = 0;
var lastSpeed = 0;
...
// things faster.
  if (lastSpeed != v) {
    segTable[activeSeg][__speed] = 0.0005+(1-v);
    lastSpeed = v;
  }
}
...
export function sliderSegments(v) {
   if (__ver > 0) return;
   var s = 1+floor(v * (__max_segments - 1));
   if (s != __n_segments) {
     __n_segments = s
     Initialize()
   }
...
// Initialize all segments to a reasonable initial value
  var def_segsize = floor(pixelCount/__n_segments);
  for (var i = 0; i < __n_segments; i++) {
    SetSegState(i,true);
    SetSegEffect(i,0);
    SetSegHSB(i,i / __n_segments,1,(i % 2)? 1 : 0.2)
    SetSegSpeed(i,1);
