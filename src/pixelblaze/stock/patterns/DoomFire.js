...
// Credit: "Doom Fire" by ZRanger1 - https://patterns.electromage.com/pattern/pHgJjpwd2XwzHrzqm
// License: Redistributed with permission from ZRanger1.
//
// The original convolution-based DOOM fire simulation includes a dragon-breath mode.
// Runs on: 2D maps; designed for panels and mapped surfaces.
// Controls: Hue — Adjusts hue; Flame Height — Adjusts flame height; Dragon Mode — Adjusts dragon mode; Speed — Adjusts speed.
...
function getWindDirection(w) {
  ...
  // Pixelblaze returns zero from a function with no explicit return value.
  return 0;
}
...
export function render2D(index, x, y) {
  x = 1 + clamp(floor(x * width), 0, width - 1);
  y = clamp(floor(y * height), 0, height - 1);
  bri = pb2[x][y];
  bri = bri * bri * bri;
