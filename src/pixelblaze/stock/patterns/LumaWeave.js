...
//
// Two crossed grayscale wave sets counter-drift through each other, weaving
// a shimmering interference field on an exact loop. A Show ingredient for
// Luma and Chroma keying rather than a finished piece: tint, scale, rotate,
// and layer it from a Show.
...
//           loop advances each wave set exactly one period;
//           Direction — Reverse, hold, or forward travel;
//           Spacing — Wave-to-wave distance; the cross set runs slightly
//           detuned so the interference keeps evolving;
//           Width — Lit fraction of each wave cycle;
//           Feather — Edge softness, from hard lattice to sine-smooth moire;
//           Lean — Wave asymmetry, from symmetric through full sawtooth,
//           leaning toward or against travel;
//           Angle — Compass origin of the primary set's travel: 0 travels top
//           to bottom, 0.25 right to left;
...
// controls) evaluated over a phase geometry - here, the product of two
// perpendicular crest fields moving in opposite directions, the cross set
// detuned to 1.18x pitch so the interference pattern never sits still. Both
// sets advance a whole period per loop, so closure is exact at any detune.
// Phase accumulates on its own clock, so animating any control from a Show
// never resets position, and one loop advances exactly one period for
// seamless BPM-locked playback. Output is pure grayscale across the full
...
export function render2D(index, x, y) {
  var dx = x - 0.5
  var dy = y - 0.5
  var u = (dx * hx + dy * hy) / pitch - phase
  var w = (dy * hx - dx * hy) / (pitch * 1.18) + phase
  hsv(0, 0, finish(crest(u) * crest(w)))
}
