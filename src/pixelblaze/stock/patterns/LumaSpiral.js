...
//
// A grayscale spiral arm winds out of the center, rotating and flowing
// outward on an exact loop. A Show ingredient for Luma and Chroma keying
// rather than a finished piece: tint, scale, rotate, and layer it from a
// Show.
...
//           loop advances exactly one winding;
//           Direction — Inward-clockwise, hold, or outward-counterclockwise
//           travel;
//           Spacing — Winding-to-winding distance;
//           Width — Lit fraction of each winding cycle;
//           Feather — Edge softness, from a hard spiral to sine-smooth
//           swells;
//           Lean — Winding asymmetry, from symmetric through full sawtooth,
...
// controls) evaluated over a phase geometry - here, radial distance plus
// angle in whole turns, which folds rotation and outward flow into a single
// spiral phase. The angle term enters in whole periods, so the angular wrap
// is seamless at any Spacing. Phase accumulates on its own clock, so
// animating any control from a Show never resets position, and one loop
// advances exactly one period for seamless BPM-locked playback. Output is
// pure grayscale across the full 0..1 range so key Target, Tolerance, and
