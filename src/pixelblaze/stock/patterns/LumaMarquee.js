// Pattern: Luma Marquee
// Built with PXLBLZ-IDE https://pxlblz-ide.whiteroomsoftware.com/
//
// Grayscale bulbs chase along the strip in wiring order on an exact loop. A
// Show ingredient for Luma and Chroma keying rather than a finished piece:
// tint, scale, rotate, and layer it from a Show.
// Runs on: 1D strips and rings, and 2D maps in wiring order; in a routed
// Show Zone the chase follows the Zone's own wiring walk, so it rides bands,
// legs, and multi-range runs.
// Controls: Loop Interval — Exact cycle length in seconds, up to 10 (the raw
//           slider value is seconds divided by 10); one
//           loop advances the chase exactly one bulb pitch;
//           Direction — Reverse, hold, or forward travel;
//           Spacing — Bulb-to-bulb distance along the run;
//           Width — Lit fraction of each bulb cycle;
//           Feather — Edge softness, from hard bulbs to sine-smooth swells;
//           Lean — Bulb asymmetry, from symmetric through full sawtooth,
...
// controls) evaluated over a phase geometry - here, normalized position along
// the wiring run, index / pixelCount. Inside a compiled Show a routed Zone
// re-binds both names to its local walk, so the same chase circles an arch
// band or climbs paired columns without knowing the map. Phase accumulates on
// its own clock, so animating any control from a Show never resets position,
// and one loop advances exactly one period for seamless BPM-locked playback.
// Output is pure grayscale across the full 0..1 range so key Target,
// Tolerance, and Softness have the whole domain.
...
export function beforeRender(delta) {
  ...
  // The tightest pitch keeps several pixels per bulb on a 250-pixel run; the
  // widest turns the whole run into one traveling bulb.
  pitch = 0.02 + spacing * 0.98
}
...
export function render(index) {
  var p = index / pixelCount / pitch - phase
