...
;(() => {
  ...
  let busy = false
  const submitRequest = async (utterance, priorHistory, captured) => {
    ...
    try {
      const response = await fetch(`${BRIDGE}/utterance`, {
        ...
        body: JSON.stringify({ requestId, show, utterance, history: priorHistory, context: editorFocusContext(captured.context) }),
