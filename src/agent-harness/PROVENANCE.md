...
| Explicit window unbounded | `telemetry/measure.ts`, `telemetry/harness.ts`, `mcp/showsServer.ts` | The explicit window had only a lower bound and the MCP schema accepted any positive number; a large request ran the synchronous frame loop until the worker died. | Explicit windows clamp to the advertised 1–600 s at every entry (the report's `input.durationMs` states the effective window); fps must be an integer in 1–240; non-finite values return `reason: 'invalid-options'` before any Pattern code runs, and `runTelemetry` applies the same envelope through the shared `resolveTelemetryBounds` (repair 2 below). |
| Partial final bucket counted as a full second | `telemetry/harness.ts` | Every bucket counted as one second, so 4.1 s of darkness reported a 5 s dark/static event. | Dark/static runs use the seconds their buckets actually cover, frame-quantized (`frames / fps`): 4.1 s is no event, 5 s is, 5.5 s reports 5500 ms and the summary says "5.5 seconds"; a dark-stretch's `darkFraction` is frame-weighted (repair 6 below). The flicker gate is unchanged. |
...
### Repairs after the candidate review (a4e11cc0)

The candidate review of the corrections above found six defects in them (two P1, three P2, one
P3). Each was repaired on 2026-09-05 with a V2-authored suite that failed against `a4e11cc0`
first; the first correction's behaviour is recorded here so it is not mistaken for the current
one.

| # | Finding | Files | Before (a4e11cc0) | After |
| --- | --- | --- | --- | --- |
| 1 | Identity judged by final-record slot | `grammar/operations/generic.ts` | `identityPreservationIssue` compared before/after array slots, so removing marker A then renaming the survivor B to A read as "A kept, B removed" and passed; a combined insert+remove+reorder was refused as a rename. | The rules above are enforced per operation against the working record with a per-patch ledger of removed ids; `identityPreservationIssue` is gone. A subtree write that would introduce an element is refused naming both the new id and the ids it would drop; insertion is `add` at an array position. |
| 2 | Raw entry unbounded | `telemetry/harness.ts`, `telemetry/measure.ts` | `runTelemetry` checked finite-and-positive only: `durationMs: 1e12, fps: 240` ran 240 billion frames. `measureShowDocument` clamped a negative window up to 1 s while the MCP schema refused it. | `resolveTelemetryBounds` (harness) is the single envelope: finite positive windows clamp to 1–600 s, non-finite or non-positive windows and any fps outside the integers 1–240 are refused, before `loadPattern`. `measureShowDocument`, the MCP tool and the raw entry all apply it; a non-positive explicit window is now `invalid-options`. |
| 3 | `finish_turn` committed inside `agent.run` | `experiment/turn.ts`, `grammar/session.ts` | `finishTurn` committed (or rolled back) immediately, so an agent that finished and then returned `incomplete`, threw, or finished a second time left a committed entry behind an incomplete turn or overwrote the recorded outcome with "nothing applied". | `finishTurn` validates through the new `validatePending` (same issues as a commit, no commit) and stages the outcome; the runner commits or rolls back only after the agent returns normally, an incompletion or exception after a finish discards the turn, and a second `finish_turn` is refused. |
| 4 | Critique compared raw stock ids | `shows/critique.ts` | Adjacent repetition, dimensional-fit de-duplication and the distinct-source count keyed on `pattern.id` as written, so `DoomFire` beside `DoomFireV20_2D` was not a repetition and counted as two sources. | Every source key applies `resolveStockPatternId`; a Show with the retired id critiques exactly as one with the successor. |
| 5 | Zone filter before time validation | `grammar/read.ts` | A junction query for a Zone with no junctions returned "none" before checking that `at_ms`/`at_playhead` was given; the clip branch likewise returned "none" for an unknown Zone before checking hovered/selected/playhead pointers. | Argument validation runs first for both branches; a well-formed miss still resolves to none with the nearest candidates. |
| 6 | `darkFraction` averaged buckets equally | `telemetry/harness.ts` | Five full dark seconds plus a 0.5 s bucket at 90% reported 0.983333. | The fraction is dark samples over all samples in the stretch (frame-weighted): 0.990909 for that case; whole buckets are unchanged. |

V2-authored suites for these repairs: `test/genericIdentityBoundary.test.ts`,
`test/telemetryBounds.test.ts`, `test/dictationTurnStagedFinish.test.ts`,
