...
const CAPTURES: PhaseCapture[] = [
  ...
  capture('v1-delivered-candidate', 'candidate', 'v1', 'delivered', 9),
  capture('v1-normalized-candidate', 'candidate', 'v1', 'normalized', 10),
  capture('v2-delivered-candidate', 'candidate', 'v2', 'delivered', 11),
  capture('v2-normalized-candidate', 'candidate', 'v2', 'normalized', 12),
  capture('v2-restored-candidate', 'candidate', 'v2', 'delivered', 13),
]
...
describe('which controls a comparison is allowed to see', () => {
  ...
  it('offers a within-version restoration comparison only that version group', () => {
    const [planned] = classifySurfaceNoisePlan(input({
      comparisons: [{
    ...
    expect(planned.plan).toEqual(['v2-delivered'])
