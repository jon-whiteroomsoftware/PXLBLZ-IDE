...
it('qualifies all actual thirty-second browser states before capture: linked deletion, final empty, dormant re-add',()=>{
 ...
 const last=editShowTransitionV2(current,{kind:'delete-clip',clipId:initial.id});expect(last.status).toBe('changed');current=last.record;expect(captured().prepared.status).toBe('empty');expect(current.composition.propertyTracks).toEqual([])
 const added=createShowClipV2(current,{kind:'create-clip',patternReference:{kind:'user',id:'deletion-voice'},runtime:{kind:'existing',instanceId:'instance'},clip:{id:'fresh',zoneId:initial.zoneId,layerId:initial.layerId,startMs:0,durationMs:30000,zoneSampleMode:'span',entryPolicy:'continue',appearance:{keys:[{id:'fresh-key',timeMs:0,value:{opacity:1,view:{mirror:false,phase:0,brightness:1},effects:[]}}]}}});expect(added.status).toBe('changed');current=added.record;expect(captured().prepared.status).toBe('ready');expect(current.composition.patternInstances).toEqual(opened.record.composition.patternInstances)
})
