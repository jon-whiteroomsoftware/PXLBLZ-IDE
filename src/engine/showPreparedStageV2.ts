  ...
  try {
    ...
    for (const instance of instances) {
      ...
      const source = ref.kind === 'stock'
        ...
        : dependencies.patterns.find(pattern => pattern.id === ref.id)?.src
      if (source !== undefined) sources[instance.id] = source
    }
    const libraries = compileLibraries(LIBRARIES, dependencies.libraries)
    ...
    if (!map) {
      ...
      const profile = dependencies.profiles.find(candidate => candidate.id === snapshot.targetControllerProfileId) ?? dependencies.profiles[0]
    ...
    return { status: 'ready', bundle: {
      record: snapshot, identity: { record, dependencies }, digest: showStageRecordDigestV2(snapshot), libraries, recipe: prepared.recipe, provenance: prepared.provenance, artifact,
