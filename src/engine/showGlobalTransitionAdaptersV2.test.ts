...
function frames(recipe:ShowRecipe,mode:'fast'|'fidelity',record?:ShowRecordV2){
 ...
 return [125,250,375,400,425,500,575,600,625,750,875].map(time=>{const frame=runtime.advanceTo(time,{stepMs:25,forceFullIntermediateRender:true});const state=Object.fromEntries(Object.entries(frame.exports).filter(([name,value])=>/elapsed|calls/.test(name)&&typeof value==='number'));expect(Object.keys(state).length).toBeGreaterThan(0);return {frame:Array.from(frame.frame),state}})
}
