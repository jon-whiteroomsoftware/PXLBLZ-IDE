...
function resolveAndLowerShowV2(
  ...
  const compileRecord: ShowRecordV2 = withoutUnusedInstanceTracksV2(expanded)
...
function resolveShowV2CompileContext(
  ...
  if (participantWindowBlockedV2(record)) {
    ...
    // sections (see `participantSectionBoundaries`). A participant Transition
    // window is not a section: the legacy composition lowerer requires both of
    // its placements in one derived Scene, so a boundary at or inside a window
    // has no representation and refuses rather than moving the activation. The
    // gate above owns the refusal condition; the lookup below only names the
    // window for the message.
    const blocked = participantTransitionWindows(record).find(window => (
  ...
  if (!flatEligible && (composition.transitions.length === 0 || wholeOutput || sectionedParticipantRoute)) {
    const unsupportedTrackIndex = firstCrossSectionTrackIndex(record, sectionedParticipantRoute ? 'transition' : 'global-sections')
...
function derivedSections(record: ShowRecordV2, route: PreparationRoute = 'global-sections'): DerivedSection[] {
  const boundaries = route === 'transition' ? participantSectionBoundaries(record) : globalSectionBoundaries(record)
...
function firstCrossSectionTrackIndex(record: ShowRecordV2, route: PreparationRoute = 'global-sections'): number {
  const sections = derivedSections(record, route)
...
function emitResolvedShowV2(context: ResolvedShowV2CompileContext): LoweredShowCompositionV2 {
  ...
  // Section-scoped activation uses the same derived-section representation as
  // the Transition-free route; the participant Transitions ride inside the one
  // section that owns their complete window. Whole-Show activation keeps the
  // existing single-section emission and its generated bytes.
  if (hasSectionScopedTrackActivationV2(context.record)) return lowerGlobalClipsToSections(context)
...
function lowerGlobalClipsToSections(
  ...
  const { record, lookup } = context
  const composition = record.composition
  const sections = derivedSections(record, context.route)
  ...
  for (const track of composition.propertyTracks.filter(track => (
    ...
    const pieces = sections.flatMap(section => {
      ...
      // Clip pieces hold at the Scene edge through an adjacent whole-output
      // Transition (#1103). An instance piece may do so only when activation
      // ends with the window and no authored key moves inside its tail (#1109).
      // Otherwise both Scenes assign one runtime in the window, so the
      // instance piece keeps its refusal. Fitting pieces preserve their bytes.
      const fits = restricted?.keyframes.every(key => key.timeMs >= section.startMs && key.timeMs <= section.endMs)
      const heldInstanceTail = 'instanceId' in track.target
        && track.activeStartMs + track.activeDurationMs <= bounds.endMs
        && !track.keyframes.some(key => key.timeMs > section.endMs && key.timeMs <= bounds.endMs)
      const piece = restricted && !fits && ('clipId' in track.target || heldInstanceTail)
        ? restrictShowPropertyTrackV2(composition.propertyTracks, track, section.startMs, section.endMs, keyIds)
        : restricted
      return piece ? [{ section, track: piece }] : []
    })
