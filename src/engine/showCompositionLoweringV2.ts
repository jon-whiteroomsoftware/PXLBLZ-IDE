...
function lowerGlobalClipsToSections(
  ...
  for (const track of composition.propertyTracks.filter(track => (
    ...
    const pieces = sections.flatMap(section => {
      ...
      // A Clip-targeted piece whose keys reach into an adjacent whole-output
      // Transition window would leave its derived Scene; it holds its
      // section-edge value there instead, as a v1 Scene did (#1103). An
      // instance track keeps the unheld piece and its refusal: the compiler
      // applies both Scenes' assignments to one runtime during the window.
      // Pieces that already fit keep their restriction and generated bytes.
      const fits = restricted?.keyframes.every(key => key.timeMs >= section.startMs && key.timeMs <= section.endMs)
      const piece = restricted && !fits && 'clipId' in track.target
