...
function resolveShowV2CompileContext(
  ...
  if (!wholeOutput && composition.transitions.length > 0 && hasSectionScopedTrackActivation(record)) {
    ...
    // has no representation and refuses rather than moving the activation.
  ...
  const sectionedParticipantRoute = !flatEligible && !wholeOutput
    && composition.transitions.length > 0 && hasSectionScopedTrackActivation(record)
 ...
 */
function hasSectionScopedTrackActivation(record: ShowRecordV2): boolean {
...
function participantTransitionWindows(record: ShowRecordV2) {
 ...
 * generated bytes.
 */
function participantSectionBoundaries(record: ShowRecordV2): number[] {
  const composition = record.composition
  return [...new Set([
    0,
...
function emitResolvedShowV2(context: ResolvedShowV2CompileContext): LoweredShowCompositionV2 {
  ...
  if (hasSectionScopedTrackActivation(context.record)) return lowerGlobalClipsToSections(context)
