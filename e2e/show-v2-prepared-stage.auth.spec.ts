...
test('native prepared Stage retains hold/Restart and later Layout through fidelity, playback and narrow controls', async ({ page }) => {
  ...
  await page.goto(`studio/shows/${source.id}?show-v2-editor=1&capture`)
  ...
  await expect(page.getByTestId('show-editor-v2-route')).toContainText(/Reopened \.pxlshow v2 and \.epe/)
