...
export async function exerciseShowV2GroupCreation(page: Page) {
  ...
  const route = page.getByTestId('show-editor-v2-route'), editor = route.getByRole('region', { name: 'Create Group', exact: true })
  const stage = page.getByTestId('show-stage-preview'), timeline = route.getByTestId('show-timeline-read-only')
  ...
  await expect(timeline.locator('[data-show-group-occurrence]')).toHaveCount(2)
  ...
  await route.getByRole('group', { name: 'Show history' }).getByRole('button', { name: 'Undo', exact: true }).click(); await expect(route.getByText('Undo saved.', { exact: true })).toBeVisible(); await expect.poll(() => settled).toBe(2)
  await expect(editor.getByRole('checkbox')).toHaveCount(3); await expect(timeline.locator('[data-show-group-occurrence]')).toHaveCount(0)
  await route.getByRole('group', { name: 'Show history' }).getByRole('button', { name: 'Redo', exact: true }).click(); await expect(route.getByText('Redo saved.', { exact: true })).toBeVisible(); await expect.poll(() => settled).toBe(3)
  await expect(timeline.locator('[data-show-group-occurrence]')).toHaveCount(2)
  ...
  await page.reload(); await expect(stage).toBeVisible(); await expect(timeline.locator('[data-show-group-occurrence]')).toHaveCount(2); expect(writes).toBe(3)
