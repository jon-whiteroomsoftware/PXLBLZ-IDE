...
test.describe('authenticated Show authoring', () => {
  ...
  test('fits the Stage without distortion while the divider reaches both height limits (#1006)', async ({ page }) => {
    test.slow()
    ...
    const geometry = () => frame.evaluate((element) => {
      ...
      return {
        // The rendered canvas rounds dimensions to integer CSS pixels.
    ...
    await expect.poll(async () => (await geometry()).aspectError).toBeLessThanOrEqual(1)
  ...
  test('keeps compact transport above the divider while timeline rows scroll (#1006)', async ({ page }) => {
    ...
    await expect.poll(async () => page.getByTestId('show-timeline-pane').evaluate((pane) => {
      ...
      if (!scroll || !toolbar) return Number.POSITIVE_INFINITY

      const scrollRect = scroll.getBoundingClientRect()
      const visibleBottom = Math.min(scrollRect.bottom, pane.getBoundingClientRect().bottom)
      const visibleRects = [toolbar.getBoundingClientRect()]
...
test('lesson pointer toggles release focus for Space playback and preserve keyboard Enter (#978)', async ({ page }) => {
  ...
  await expect(guide).toBeHidden()
  await expect(pause).toBeVisible()
  if (process.env.PXLBLZ_FOCUS_CAPTURE) await page.screenshot({ path: process.env.PXLBLZ_FOCUS_CAPTURE })
  await page.keyboard.press('Space')
  await expect(play).toBeVisible()
  ...
  await expect(toggle).toBeFocused()
  await expect(play).toBeVisible()
  const search = page.getByRole('textbox', { name: 'Search shows', exact: true })
...
test('Show strip summaries retain controls, keyboard ownership and canvas at the usable width clamp (#968)', async ({ page }) => {
  ...
  for (const name of ['Stage', 'Zones', 'Source code']) await expect(controls.getByRole('button', { name, exact: true })).toHaveAttribute('aria-expanded', 'false')
  await expect(page.getByTestId('show-compile-bar')).toHaveCount(0)
  await expect(strip.getByText(/show paused|previewing Show/)).toHaveCount(0)
  const brightness = strip.getByRole('slider', { name: 'Show preview brightness', exact: true })
  ...
  await expect(brightness).toHaveAttribute('aria-valuetext', '0%')
  await brightness.press('End')
  await expect(brightness).toHaveAttribute('aria-valuetext', '100%')
  ...
  await controls.getByRole('button', { name: 'Source code', exact: true }).click()
  await expect(page.getByTestId('show-source-outlet')).toContainText('Pattern copies running')
  await controls.getByRole('button', { name: 'About Pattern copies running' }).focus()
  await expect(page.getByRole('tooltip')).toContainText('Busiest LED')
  await controls.getByRole('button', { name: 'About Pattern copies running' }).press('Escape')
  ...
  await expect(stageSection).toHaveAttribute('aria-expanded', 'true')
  await expect(controls.getByRole('button', { name: 'Source code', exact: true })).toHaveAttribute('aria-expanded', 'true')
  await previewSection.click()
  ...
  const sourceBody = page.getByTestId('show-source-outlet').locator('.show-source-inventory-body')
  await expect(sourceBody.locator('[data-source-resource]')).toHaveCount(3)
  expect(await sourceBody.evaluate((body) => {
    const bounds = body.getBoundingClientRect()
    return [...body.querySelectorAll<HTMLElement>('[data-source-resource]')].every((row) => {
      const label = row.firstElementChild!.getBoundingClientRect()
      const value = row.querySelector<HTMLElement>('[data-source-resource-value]')!
      const rect = value.getBoundingClientRect()
      return Math.abs(label.top - rect.top) < 1 && rect.right <= bounds.right + 1
        && rect.left >= label.right && value.scrollWidth <= value.clientWidth
    })
  })).toBe(true)
...
test.describe('lesson pill, Reading card and Live strip (#985)', () => {
  ...
  test('chooser Reset preserves Clip edits and Undo while restoring Pattern picks (#987)', async ({ page }) => {
    await page.setViewportSize({ width: 1440, height: 900 })
    await page.goto('studio/shows/stock-show-102-transitions-values')
    await pauseAtStart(page)
    const chip = page.getByRole('button', { name: 'Patterns (3)' })
    const popover = page.getByRole('dialog', { name: 'Try with Pattern', exact: true })
    const reset = popover.getByRole('button', { name: 'Reset', exact: true })
    const clip = page.getByRole('button', { name: 'Select ClockworkIris', exact: true })
    const detail = page.getByRole('dialog', { name: 'Entity Detail Panel' })
    const brightness = detail.getByRole('textbox', { name: 'Brightness exact percentage' })
    const undo = page.getByRole('button', { name: 'Undo Show edit' })

    await clip.click()
    const authoredBrightness = await brightness.inputValue()
    expect(authoredBrightness).not.toBe('37')
    await brightness.fill('37')
    await brightness.press('Enter')
    await page.keyboard.press('Escape')
    ...
    await expect(popover).toBeHidden()
    await chip.click()
    await expect(picker).toHaveValue('Caustics')
    ...
    await undo.click()
    await clip.click()
    await expect(brightness).toHaveValue(authoredBrightness)
  })
...
test('Stage outlines follow selection and Zone Layout navigation (#983)', async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 1000 })
  await page.goto('studio/shows/stock-show-showcase-zone-layouts-stripes-grid')
  const rail = page.getByRole('group', { name: 'Show preview controls' })
  await expect(rail).toBeVisible()
  const pause = rail.getByRole('button', { name: 'Pause Show preview', exact: true })
  if (await pause.isVisible()) await pause.click()
  await page.getByRole('button', { name: 'Go to Show start', exact: true }).click()
  const clock = page.getByRole('status', { name: 'Show time' })
  await expect(clock).toContainText('00:00.0')
  await rail.getByRole('button', { name: 'Show Zone outlines', exact: true }).click()
  await rail.getByRole('button', { name: 'Show Selected Clip outline', exact: true }).click()
  const zones = page.getByTestId('show-stage-zone-outlines').locator('rect')
  const clip = page.getByTestId('show-stage-clip-outline')
  await expect(zones).toHaveCount(1)
  await page.getByRole('button', { name: 'Select IQPalettes', exact: true }).last().click()
  await page.getByRole('button', { name: 'Close Entity Detail Panel', exact: true }).click()
  await expect(clip).toHaveCount(0)
  const playhead = page.getByRole('slider', { name: 'Show playhead', exact: true })
  await playhead.press('End')
  await playhead.press('ArrowLeft')
  await expect(clock).toContainText('00:14.0')
  await expect(zones).toHaveCount(4)
  await expect(clip).toBeVisible()
  const polygon = clip.locator('polygon')
  const gridBounds = await polygon.getAttribute('points')
  expect(gridBounds).toMatch(/^0\.5\d+,0\.0000 1\.0000,0\.0000/)
  expect(Number(await polygon.getAttribute('stroke-width'))).toBeGreaterThanOrEqual(1)
  expect(Number(await zones.first().getAttribute('stroke-width'))).toBeGreaterThanOrEqual(1)
  await expect(page.getByRole('status', { name: 'Rebuilding Show preview' })).toHaveCount(0)
  const gridCapture = test.info().outputPath('983-grid-ember.png')
  await expect(page.getByTestId('show-stage-canvas-frame')).toHaveAttribute('aria-busy', 'false')
  await page.getByTestId('show-stage-canvas-frame').screenshot({ path: gridCapture })
  await test.info().attach('Grid Ember outline', { path: gridCapture, contentType: 'image/png' })
  await rail.getByRole('button', { name: 'Hide Selected Clip outline', exact: true }).click()
  await expect(clip).toHaveCount(0)
  await rail.getByRole('button', { name: 'Show Selected Clip outline', exact: true }).click()
  await expect(polygon).toHaveAttribute('points', gridBounds!)
  await expect(clock).toContainText('00:14.0')
  await expect(rail.getByRole('button', { name: 'Play Show preview', exact: true })).toBeVisible()
  await playhead.press('Home')
  for (let index = 0; index < 5; index++) await playhead.press('ArrowRight')
  await expect(clock).toContainText('00:05.0')
  await expect(zones).toHaveCount(4)
  expect(Number(await zones.first().getAttribute('width'))).toBeLessThan(0.3)
  await expect(clip).toHaveCount(0)
  await playhead.press('Home')
  await expect(zones).toHaveCount(1)
  await page.getByRole('treeitem', { name: 'Quadrille', exact: true }).click()
  await expect(clip).toHaveCount(0)
})

test('Quadrille outlines stay readable across pane sizes and content brightness (#983)', async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 1000 })
  ...
  await page.getByRole('button', { name: 'Go to Show start', exact: true }).click()
  const playhead = page.getByRole('slider', { name: 'Show playhead', exact: true })
  for (let index = 0; index < 7; index++) await playhead.press('ArrowRight')
  await expect(page.getByRole('status', { name: 'Show time' })).toContainText('00:07.0')
  await expect(page.getByRole('status', { name: 'Rebuilding Show preview' })).toHaveCount(0)
  const stage = page.getByTestId('show-stage-canvas-frame')
  const polygon = page.getByTestId('show-stage-clip-outline').locator('polygon')
  const capture = async (name: string) => {
    await expect(page.getByRole('button', { name: 'Close Entity Detail Panel', exact: true })).toHaveCount(0)
    await expect(stage).toHaveAttribute('aria-busy', 'false')
    const path = test.info().outputPath(`${name.replaceAll(' ', '-')}.png`)
    await stage.screenshot({ path })
    await test.info().attach(name, { path, contentType: 'image/png' })
  }
  await page.getByRole('button', { name: 'Select WavyBands', exact: true }).nth(5).click()
  await page.getByRole('button', { name: 'Close Entity Detail Panel', exact: true }).click()
  await capture('Quadrille off')
  await rail.getByRole('button', { name: 'Show Zone outlines', exact: true }).click()
  await rail.getByRole('button', { name: 'Show Selected Clip outline', exact: true }).click()
  await expect(polygon).toHaveAttribute('points', '0.0000,0.0000 0.5000,0.0000 0.5000,0.5000 0.0000,0.5000')
  await capture('Quadrille upper-left bright 1440')
  await page.getByRole('button', { name: 'Select WavyBands', exact: true }).nth(1).click()
  await page.getByRole('button', { name: 'Close Entity Detail Panel', exact: true }).click()
  await expect(polygon).toHaveAttribute('points', '1.0000,1.0000 0.5000,1.0000 0.5000,0.5000 1.0000,0.5000')
  await capture('Quadrille lower-right bright 1440')
  const brightness = page.getByRole('slider', { name: 'Show preview brightness', exact: true })
  await brightness.fill('0.05')
  await capture('Quadrille lower-right dark 1440')
  await page.setViewportSize({ width: 1180, height: 900 })
  await capture('Quadrille lower-right dark 1180')
  await brightness.fill('1')
  await capture('Quadrille lower-right bright 1180')
})
