...
import { ISSUE928_PIXEL_COUNTS, issue928Fixtures } from './issue928'

// ISSUE928_HARDWARE=1 PIXELBLAZE_IP=<ip> npx vitest run test/perf-harness/issue928.hardware.test.ts
const runHardware = process.env.ISSUE928_HARDWARE === '1'
const ip = process.env.PIXELBLAZE_IP ?? '192.168.8.224'
const measurementOptions = { activationTimeoutMs: 20_000, settleMs: 2_000, sampleMs: 4_000 }

describe('generated frame-constant hoist paired ladder (#928)', () => {
  it.skipIf(!runHardware)('measures pass-off against pass-on at 256/500 px and restores Controller state', async () => {
    ...
    const rows: unknown[] = []
    try {
      for (const pixelCount of ISSUE928_PIXEL_COUNTS) {
    ...
    const outputPath = join(process.cwd(), 'test/perf-harness/issue928-hoist-ladder.json')
