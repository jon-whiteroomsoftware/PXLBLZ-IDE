...
const runHardware = process.env.ISSUE527_HARDWARE === '1'
const ip = process.env.PIXELBLAZE_IP ?? '192.168.8.224'

describe('content-key composition Controller probe (#527)', () => {
  it.skipIf(!runHardware)('benchmarks paired 2,000-pixel artifacts and restores Controller state', async () => {
    const { counterfactualArtifact, ISSUE527_PIXEL_COUNT, selectedArtifact } = await import('./issue527')
    ...
    let report: unknown
    try {
      if (original.pixelCount !== ISSUE527_PIXEL_COUNT) {
        connection.setPixelCount(ISSUE527_PIXEL_COUNT, false)
        await sleep(1_000)
      }
