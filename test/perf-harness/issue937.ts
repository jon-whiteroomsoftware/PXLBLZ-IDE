...
// a full-Stage zone (cut boundaries: a snapshot Crossfade is a stateful
// cache and declines the hold), plus the light hsv-steady fixture.
...
/** One member alone in a full-Stage zone, cut boundary, 30 s hold. */
function heavy(member: string): ShowRecipe {
  ...
  return {
    ...
    routedSceneSequence: {
      scenes: [
        { holdMs: 30_000, placements: [{ placementId: 'heavy', zoneName: 'stage', clipId: 'heavy' }] },
...
export function issue937Candidates(): Issue937Candidate[] {
  ...
  for (const [name, recipe, sampleMs] of [
    ...
    ['hsv-steady-light', heavy('EasedSweep'), 4_000],
  ] as const) {
