...
const runHardware = process.env.ISSUE513_HARDWARE === '1'
const ip = process.env.PIXELBLAZE_IP ?? '192.168.8.224'
const pixelCounts = (process.env.ISSUE513_PIXEL_COUNTS ?? '256,1000,2000')
  .split(',')
...
describe('Redline frame-invariant and kernel Controller matrix (#513)', () => {
  it.skipIf(!runHardware)('benchmarks 256, 1,000, and 2,000 pixels and restores Controller state', async () => {
    ...
    try {
      ...
      for (const pixelCount of pixelCounts) {
        ...
        const diagnosticVariants = pixelCount === 2_000
          ? {
              combined: await pushAndMeasure(connection, combinedArtifact, compile),
              kernelOnly: await pushAndMeasure(connection, kernelOnlyArtifact, compile),
            }
          : {}
        measurements.push({
          pixelCount,
