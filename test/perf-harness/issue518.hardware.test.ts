...
const runHardware = process.env.ISSUE518_HARDWARE === '1'
const ip = process.env.PIXELBLAZE_IP ?? '192.168.8.224'

describe('compatible Pattern-output reuse Controller probe (#518)', () => {
  it.skipIf(!runHardware)('benchmarks the paired 2,000-pixel artifacts and restores Controller state', async () => {
    const { counterfactualArtifact, ISSUE518_PIXEL_COUNT, selectedArtifact } = await import('./issue518')
    ...
    let report: unknown
    try {
      if (original.pixelCount !== ISSUE518_PIXEL_COUNT) {
        connection.setPixelCount(ISSUE518_PIXEL_COUNT, false)
        await sleep(1_000)
      }
