...
- Diffusion is owned by the WebGL renderer (`setDiffusion`), driven by `diffusionGlow(diffusion, coreDiameterPx, pitchPx)` in `camera.ts`, which returns the grown quad size, the core's fraction of it, and the tail strength. The old SVG `feGaussianBlur` filter and the `diffusionBlurStdDev` / per-dimension `DIFFUSION_BLUR_PITCH_FACTOR_*` helpers are removed.
