...
| Mode | read-only | Read-only sweep, rendered through the production composition (`InspectorPanel` under `InspectorReadOnlyContext`, dialog `readOnly={false}` as ShowEditor passes it): every form control including Effect parameter editors and action-menu triggers disabled, no action menu can open, tabs navigable, zero patches under any interaction |
