...
The shipped transport — the framework-free, isomorphic `PixelblazeConnection` (injected WebSocket factory: browser `WebSocket` or Node `ws`), the documented JSON API, the binary-frame protocol, and both committed reports — is described in `docs/REFERENCE.md` §16.
