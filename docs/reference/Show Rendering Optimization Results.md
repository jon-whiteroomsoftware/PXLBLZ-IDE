...
| 03 | #931 member loop rewrites | `i = i + 1` loop updates become `i++` (1.7 us/iteration) and render-reachable loops with a literal or never-written-constant bound (trip <= 16, no control flow, no induction writes, no closure observing the index, 16.16-safe range) unroll with the index as a literal, under an 8 KB growth allowance and a byte-budget fallback (`loopRewrites.reason = 'artifact-byte-budget'`). Paired ladder at **256 px only** (`issue931-unroll-ladder.json`; the 500 px rung is owed against the wave-5 256/500 contract): IridescentFibers **+6.35%**, NeonSquircles **+9.74%**, PulseLoom **+9.62%**, ShaderShowcase **+4.51%** (unrolled); Kishimisu **+1.42%**, ZippyZaps **+0.85%** (idiom only). Bug found on the bench: the Controller compiler has no comma expression - later loop copies emit one statement per declarator, and an offline device-compiler acceptance test now guards every pass |
| 04 | #930 single-use temporary elimination | **parked after three blocked review rounds** (non-convergence rule): a general forward substitution in generated arms kept sprouting semantic corner cases (argument re-reads, function-scoped `var`, callee positions, array aliases, branch evaluation counts). Measured before parking (256 px only): hsv-steady +5.99 / +6.14%, effect-tax +1.98%, others under 1%, Redline byte-identical (`issue930-temporaries-ladder.json` on the parked branch). Reopen only as the narrow generated-arm shape it was built for |
| 04b | #926 hold variants (spike, no product change) | plain hold sits at the (K−1)/K ceiling on heavy members (×2 +92%, ×4 +268% on ZippyZaps and Caustics); **hold-and-lerp keeps most of it (×2 +79%, ×4 +227%) for about half the mean emulator drift** at the same K; alternating-parity anchors are free (within 1%) and trade static pixelation for shimmer; hold over Rolling Refresh not exercised (recipe-level policy did not engage on a routed-scene clip). Drift table measured on a 256-index strip of a 45×45 synthesized domain (lower bound; corrected after review). Verdict carried into #937 (`docs/plans/issue-926-hold-variants-results.md`, `issue926-variants-ladder.json`) |
| 05 | #937 spatial hold-and-lerp (compile option, off by default) | built from the #926 spike at Jon's direction, lerp only, no UI; the final dispatcher renders one stride ahead at every anchor and blends between anchors; eligible only where the dispatcher synthesizes coordinates from the index (4 of 40 stock Shows; the 36 coordinate-routed Portable Shows decline with `spatialHold.reason = 'coordinate-routed'`). Paired ladder at **256 px only** (`issue937-lerp-ladder.json`, 500 px owed against the wave-5 contract; direct sinks off in every row because the latch captures RGB): ZippyZaps **+78.7% / +227.3%** and Caustics **+78.0% / +225.7%** median FPS at x2 / x4; the light hsv-steady member +6.8% / +53.7% over its direct-sink-off baseline, which is itself below the production direct-sink build (54.5 vs 70.8 FPS) — the option pays for heavy members, not light ones. Anchors equal the baseline sample in Fast and Precise; every wrapped artifact carries exactly one native paint |
...
| 07 | #936 boundary-latched decode (exact, default on) | shared physical cut-scene dispatcher recomputes route, local-index base, zone dimensions, placement plan and configuration only at index 0 and zone boundaries (ascending render order, #560, now load-bearing and falsified by a shuffled-order test). Redline **+13.31 / +13.30%** median FPS at 256 / 500 px, bytecode -616 B (`issue936-latch-ladder.json`); the arm-chain collapse adds ~0.2 points and counter-based column/row loses 0.6, so the plan chain and the coordinate formulas stay. Catalogue: Redline and 301 latch, 302 and the Overture remix decline (non-interned plans), 36 have no shared cut dispatcher; checksum parity in both modes across all 40. Portable predictive dispatch not pursued under the <=500 px scope (`docs/plans/issue-936-boundary-latch-results.md`) |
| 08 | #934 approximate transcendentals (compile option, off by default) | cost rows: `exp` **22.1 us**, library `tanh` 46.1 us, `pow(b, 1.3)` 8.5 us, `asin`/`acos` 4.8 us, `log` 1.5 us, divide 2.2 us; substitutes: reciprocal quintic for `exp(-t)` (~13 us), fitted quadratic for a fractional `pow` on [0, 1] (4.9 us), rational `tanh` (11.8 us); **recorded negatives**: polynomial `asin` (12.3 us), 64-entry table + lerp (14.8 us), `(1 + t/16)^-16` (17.8 us). Pass `memberTranscendentalApproximation` with interval-analysis domain proofs; census: PhantomStar, PlasmaNebula, Kishimisu touched, everything else declined with a reason. Pairing at 256 px: PhantomStar **+2.3%** (0.236 -> 0.242 FPS, 90 s samples), PlasmaNebula +0.6%, Kishimisu 0% (memoized site); emulator drift max 1 / 4 / 1 of 255; contact sheets indistinguishable. Ships as disclosed breadth for ports, not a catalogue win (`docs/plans/issue-934-transcendental-results.md`) |
| 06 | #933 display-exact tier + integer-pow lowering (compile option, off by default) | tier: `qualifyDisplayExact` (max 8-bit delta 0 in both modes; checksums free to differ) with `npm run drift` printing the verdict and the residual named in `docs/agents/verification.md`. Cost rows (round five): `pow(b, 3)` / `pow(b, 4)` 7.63 us vs multiply chain 3.62 / 4.70 us with a hoisted base; `pow(b, 2)` is a firmware fast path (2.28 us) a hoisted chain loses to, so k = 2 rewrites only on a plain name. Firmware facts: negative base with integer exponent follows C powf; overflow diverges (`pow(200, 2)` 32768 vs `200 * 200` -25536), so a site needs a provable bound with bound^k <= 32767. Per-pixel fixture (6 sites) **+29.5%** median FPS at 256 px only (`issue933-pow-ladder.json`). Census: no stock per-pixel integer-pow site (Oasis' activation-time table is the only eligible one, in no stock Show), so every stock artifact is byte-identical with the option on. The fixture is checksum-exact in Fast and one 16.16 LSB short of display-exact in Precise (max delta 1) - the Precise emulator's pow fidelity bounds the tier (the bench matched the chain on every positive-base sample); recorded, threshold kept strict (`docs/plans/issue-933-pow-lowering-results.md`) |
...
`issue931-unroll-ladder.json`, `issue926-variants-ladder.json`,
`issue937-lerp-ladder.json`, `issue933-pow-ladder.json`.
`issue937-lerp-ladder.json`, `issue936-latch-ladder.json`, `issue934-approx-ladder.json`.
