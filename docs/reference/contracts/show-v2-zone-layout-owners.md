...
- **A fresh Show cannot gain a second Zone yet.** A fresh v2 Show's two Clips
  ...
  control or a fresh-Show sampling decision, both outside this slice: changing
  `showCreationV2` to `span` would move a fresh Show off the `continuous-flat`
  lowering route and change its generated bytes.
