...
Authority: scene-retirement specification §§4–9/12, assigned ROUTE/FAILURE/SHARING/RESTART/CURVE/ACTIVATION/INSERT/GROUP-HOLD/LAYOUT-END proof subset. Clip timing, Add Clip and Insert Time controls run in the one editor on the v2 backing selected by a stored v2 row or native v2 built-in. There is no preview parameter. The approved layout, supported partitions and limits are recorded in [the frontend contract](../evidence/issue-1038-clip-timing-workspace/frontend-contract.md).
