...
symbols. The ordered per-machine base ledger plus generated bytes reconciles to
each Pattern row. Pattern rows and shared generated categories reconcile to
`artifactBytes`; reserved empty members belong to routing rather than appearing
as creator-authored Patterns. The provenance and delivery header is a separate
prefix that raises `artifactBytes` to delivered source. With a selected
...
The creator-facing counts exclude reserved empty members; the internal
renderer fields still include them for compile-pressure safety. These labels
