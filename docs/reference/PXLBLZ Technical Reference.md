...
compacted UTF-8 artifact. Pattern-owned ranges distinguish symbols originating
in the lowered Pattern source (`compiled-pattern`) from member state, Controls,
renderer adapters, and placement-specific emission (`member-generated`). The
ordered per-machine base ledger plus generated bytes reconciles to each Pattern
row. Pattern rows and shared generated categories reconcile to
`artifactBytes`; the internal empty routed member belongs to routing rather
than appearing as a creator-authored Pattern. The provenance and delivery
header is a separate prefix that raises `artifactBytes` to delivered source.
With a selected Controller, active profile transforms remain one further exact
byte delta, so the final total matches the source offered to the Controller
compiler.
...
delivered code, and authored references as timeline placements. A separate
**Pattern copies running** axis reports the maximum number of compiled copies
that can run simultaneously. Its **Busiest LED** line translates per-pixel
...
These labels preserve the distinction between
`steadyStateRenderersPerController` / `worstInstantRenderersPerController` and
the per-pixel renderer fields. Hover-open inventory uses a short pointer-leave
grace period so the portaled panel remains reachable; selecting the source
meter pins it and reveals the explicit close control.
