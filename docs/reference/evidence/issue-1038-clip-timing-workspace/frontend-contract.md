...
New native row/timing presenter and pure presentation/draft-intent model, closed checked wrappers, pilot wiring, own tests/contracts/evidence. Pure create/Clip/Transition/Group/time/Layout transformations, schema/compiler/provider/history and frozen trees untouched. The approved narrow Worker Group structural-recursion prerequisite is documented in ../../contracts/show-v2-worker-group-admission.md; it retains structural-first and mandatory domain validation. Whole-Clip appearance/Replace/independence, Layer CRUD, Group lifecycle/isolation/create, Property CRUD and pointer manipulation are later named adoption work.
