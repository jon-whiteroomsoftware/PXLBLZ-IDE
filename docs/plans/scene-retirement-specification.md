...
| Leading resize | Keep the selected Clip's end fixed. At an incoming Transition, changing its start changes incoming duration by the same delta while the outgoing end stays fixed. A zero or negative result removes the Transition in the same edit, with no ripple: the Clip's start moves to the requested start and nothing else moves (Jon, 2026-09-24, #1111-C). Scalar Property ramps on the removed Transition are projected when the gesture supplies a projection plan (the timeline edge drag through `planShowTimelineGestureV2`, which plans one when the drag reaches or passes the window start), and the edit refuses when it does not (the editor's `resize-leading` intent and the agent `resize_clip`). Update whole-output contributor consistency atomically or refuse. |
...
| A lossy edit asks first on v2 and v1 acts silently. Replace Pattern, a cross-Layer Clip move that removes connected Transitions, unticking a control target that has a lane, and a whole-Clip appearance write over held segments open a confirmation that names what the edit removes or overwrites; Cancel writes nothing. The Source Pattern picker marks each Pattern whose selection would remove controls. Group Clips confirm the same way against their Group definition | #1069 (Jon, 2026-09-23): v2-only confirmations |
...
| A leading resize that reaches or passes its incoming Transition's window start extends the Clip to the requested start and removes the Transition, with its Property ramps and contributor sets, in the same edit. Nothing else moves and Show End stays fixed; only a result that fails validation refuses, with the validator's message | Jon, 2026-09-24 (#1111-C); §5 Transition edits |
