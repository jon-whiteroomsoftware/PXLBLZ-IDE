...
Status: completed and archived. Issues #579-#589 and #592 implemented the
unified editor and retired the legacy Scene-centric UI. This document preserves
the approved product intent and implementation-era decisions; it is not the
source of current behavior or remaining work.

The shipped model deliberately differs from one early statement below: Scenes
disappeared from authoring, not from persistence and compiler lowering.
`ShowCompositionV1` still uses internal Scene partitions as a compatibility
representation. Current truth lives in `CONTEXT.md`, the Feature Guide, and the
Technical Reference.
