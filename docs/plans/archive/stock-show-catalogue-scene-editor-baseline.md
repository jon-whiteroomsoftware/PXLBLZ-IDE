# Stock Show catalogue build packet - Scene-editor baseline

Status: superseded and archived. The fifteen-lesson unified-editor curriculum in
[`../stock-show-catalogue-build-packet.md`](../stock-show-catalogue-build-packet.md)
replaces this sequence. Preserve this document only for its fixture evidence,
visual-budget discipline, note style, and implementation history.

Status: redesign required before implementation resumes. The production
catalogue plumbing, Show-note disclosure, and initial fixtures exist, but this
course was written for the retired Scene-centric editor and was set aside before
its first curriculum review. Its progression, writing style, visual budget, and
review discipline remain strong design inputs. Its lesson list, fixtures, note
copy, guide targets, screenshots, and implementation order are not an approved
launch catalogue.
...
## Unified-editor redesign gate

The Show editor now teaches a different visible model. Authors work directly
with Clips on Layers and Zones under one ruler; Scenes remain internal
persistence and compiler partitions. The course must not mention opening a
Scene editor, reading a Scene X-ray, or using Scene-local scope. Internal Scene
and composition terms may remain in fixture specifications only when they are
needed to construct or verify the persisted record.

The redesign should preserve:

- the foundations-to-composition progression;
- one dominant idea and visual change per lesson;
- executable Shows rather than prose-only tutorials;
- concise notes with one Notice, two experiments, and one guide handoff;
- calm diagnostic sources before visually dense material; and
- separate automated, capture, and human-review gates.

The first design review must decide:

1. how the original ten lessons map onto the direct timeline;
2. which new lessons teach Clip placement and Viewport, Pattern-instance
   lifecycle, Freeze/Strobe/Blink/Stutter, Groups, global timing tools, and
   changing Zone Layout intervals;
3. whether the expanded vocabulary needs a third course level rather than
   overloading the existing 100 and 200 series;
4. which Effects and Transition families still deserve standalone lessons or
   showcase coverage;
5. which source Patterns and complete Show recipes make the new mechanisms
   visually legible and worth publishing;
6. how current compile-cost, output-contract, export, Run, and Save behavior
   should appear in the learning path; and
7. which existing fixtures remain useful implementation evidence even when
   their lesson framing is replaced.

Until that review is approved, the catalogue structure and recipes below are
the original baseline for comparison, not build instructions. Issue #363 owns
the eventual accepted curriculum and implementation state.
...
This sequence predates the unified editor and must not resume unchanged. Replace
it only after the revised curriculum has passed the first design review.
...
The approved note disclosure remains reusable. Curriculum redesign is the
remaining product-definition blocker before catalogue implementation resumes.
