...
Two independent per-point channels. **sample** — the coordinates fed to the render fn (`[x]` for 1D, `[x,y]` for 2D, `[x,y,z]` for 3D); always owned by the **map coordinate view**. With no installed 1D map, the implicit **Index** view supplies `x = index / pixelCount`. **pos** — where the dot is *drawn* (a 2D or 3D point). `pos` is **dual-sourced**: *map-intrinsic* when the map encodes real geometry (grid, cube, a measured installation), but *viewport-supplied* when a separate embedding owns presentation. They coincide for a flat grid; they diverge for a ring (sample `[x]` from Index or a 1D map, pos a 2D circle supplied by a **Shape**) or a 2D map wrapped onto a cylinder (sample `[x,y]` from the map, pos 3D supplied by a **Surface**). The viewport-supplied case spans both 1D shapes and 2D surfaces.
