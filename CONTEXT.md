...
**Left rail**:
The app's object browser and primary open/select surface. Its top-level organization follows **object type** before provenance: **Patterns**, **Maps**, **Mixins**, **Libraries**, **Controllers**, and **Shows** live as rail modes, with **Catalog** as the bottom entry for the public Gallery browse surface. **Patterns**, **Maps**, **Mixins**, and **Libraries** list user-owned rows first and then an always-present, collapsible stock/built-in section; those sections remember their open/closed state during the session. **Controllers** lists durable hardware profiles by their last observed device names, has no stock section, and offers deletion but no blank-new or rename affordance. The **dimension lens** and type-down name search belong only to dimensional authoring content (Patterns and Maps), not Controllers, Mixins, or Libraries. Switching rail mode restores that mode's last selected/open destination where possible.
...
**Show**:
A saved choreography of scenes across zone rows. Its canonical editor is a proportional timeline: scenes are duration-sized column headers, zones are rows, and stable first-class **transition** entities occupy one shared boundary lane. Cut, crossfade, wipe, dither, portal, and routing markers select independently of either neighboring scene; even a zero-duration cut is an explicit boundary entity. A **clip** references a pattern/demo, occupies one or more scenes and zones, applies adaptations, and owns explicit entry behavior: **Continue** reuses matching private Pattern state across the boundary, while **Restart** begins a fresh instance and time base. Splitting at the playhead creates a shared scene boundary and defaults every destination clip to Continue. A property target belongs to its destination clip; the boundary owns how an explicit start reaches it over that property's duration and easing. Nested Time, Brightness, and active public Pattern-control lanes are projections of the same property-transition record; multiple independent curves may run on one continued Pattern member, including exact-zero pause without private-clock reset. Pattern controls mean exported slider functions only, never arbitrary private locals. The Show compiler turns the arrangement into one generated Pixelblaze artifact routed over Controller zones or a selected stage. Shows own transport, timing, placement, zone routing, and clip adaptation so patterns remain reusable textures. Preview seeking reconstructs deterministic Pattern state from Show start in the Fast runtime; it does not mutate the saved Show.
...
A stable entity owned by a scene boundary, separately selectable from either neighboring scene or clip. A transition carries a visual/routing kind, duration, easing, and type-specific configuration; a zero-duration cut is still an explicit transition. Destination clips own property targets, while the incoming transition owns how explicit start values reach those targets. Visual effects and routing-layout markers may coexist at one boundary and share the same lane/selection vocabulary without becoming one runtime policy.
