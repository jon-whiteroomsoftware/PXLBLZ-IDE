...
The storage seam behind **Your patterns** and **Your maps**. Exactly one provider is active at a time. The default provider is browser-local IndexedDB in public builds and localhost development. A localhost-only workspace provider can be explicitly enabled for development; when active, personal patterns/maps are read from and written to repo-root files under `personal/patterns/` and `personal/maps/`, while `personal/controllers/` and `personal/bindings/` are reserved future homes. Existing IndexedDB records are not migrated into files automatically.
_Avoid_: treating the workspace provider as the long-term backend; implying both browser and workspace personal stores are visible at once.
