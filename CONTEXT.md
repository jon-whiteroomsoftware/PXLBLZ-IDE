...
**Template** (stock-map):
The verbatim plain-JS **source** of a source-backed **stock map** (Square, Wide 2:1, Cube, Star, Ring, sphere/helix clouds), loaded into the **New Map** editor as a starting point for a new **custom map**. The **only** way to see stock-map code — stock maps are never openable in place; loading a template forks an editable custom copy. A template load copies **source text only** — never name (user-set) or `dim` (inferred at bake from the returned arity). Browsable from a **"Load template" dropdown** in the New Map editor: loading is silent when the buffer is pristine (byte-identical to the last-loaded baseline — skeleton or a previously-loaded template) and **confirms before overwriting** once edited.
_Avoid_: seed (reserved for the example clouds' historical IDB rows); preset.

**Recommended map** (demo → map association):
A purely **IDE-side**, preview-only pointer from a **demo** to the **stock map** it is meant to be viewed on (e.g. the sphere-rings demo recommends the Sphere cloud). It is *not* part of the Pixelblaze universe — the physical **Controller** knows only patterns and maps, never associations — and it lives **nowhere in the pattern source or the transpiled artifact**: it is a small registry in the demo-loading layer (`demoName → mapId`), consumed only by the **Layout** resolver as the default when a pattern has no persisted layout selection. The same category as the existing per-pattern layout selection (preview-only, never sent to hardware), just table-sourced. The map stays freely switchable; the recommendation only sets the on-open default. Applies to **demos** (read-only, no `PatternRecord`); a **custom pattern** persists its own selection instead.
_Avoid_: baking the association into pattern source or a `//@map` comment (would pollute the artifact); "required map" / "bound map" (it is only a default, never a constraint).
...
The 3D form a **2D map's** flat field is *drawn* on — **flat** (the identity surface, today's plain 2D preview), **cylinder**, **sphere**, torus, polyhedron nets (a growing family). The 2D sibling of **Shape**: it owns `pos` (where each dot is drawn in 3D) while the **map** keeps owning `sample` (the `[u,v]` the pattern reads) — the §**Sample / position** divergence, made first-class. A surface consumes the map's aspect to set its geometry (a square field wraps to a tall slender cylinder, a 2:1 field to a fatter shorter one). Because a 2D pattern's `sample` is *non-empty*, a surface must be **composed with a source map** — which is why 2D shows two controls (**Map** + the embedding dropdown, here populated with surfaces) where 1D shows one. "Flat" is just the trivial member, so today's plain 2D preview is the identity entry of this family, not a separate kind. The **cylinder** is therefore an embedding, **not** a map (ADR-0010, superseding the ADR-0009/0008 "drape cylinder is a stock-map exception" framing — see those ADRs' cylinder notes).
_Avoid_: calling a surface a "map" (it owns `pos`, not `sample`); "drape" / "drape cylinder" (the cylinder is one surface among several now); reusing "shape" for it.
