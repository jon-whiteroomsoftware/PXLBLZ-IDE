...
**Show**:
A saved choreography of scenes across zone rows. Its canonical editor is a proportional timeline: scenes are duration-sized column headers, zones are rows, and stable first-class **transition** entities occupy one shared boundary lane. Cut, crossfade, wipe, dither, portal, and routing markers select independently of either neighboring scene; even a zero-duration cut is an explicit boundary entity. A scene cell references a pattern/demo plus adaptations and owns explicit entry behavior: **Continue** reuses matching private Pattern state across the boundary, while **Restart** begins a fresh instance and time base. Splitting at the playhead creates a shared scene boundary and defaults every destination cell to Continue. A property target belongs to its destination cell; the boundary owns how an explicit start reaches it over that property's duration and easing. Nested Time, Brightness, and active public Pattern-control lanes are projections of the same property-transition record; multiple independent curves may run on one continued Pattern member, including exact-zero pause without private-clock reset. Pattern controls mean exported slider functions only, never arbitrary private locals. The Show compiler turns the arrangement into one generated Pixelblaze artifact routed over Controller zones or a selected stage. Shows own transport, timing, placement, zone routing, and clip adaptation so patterns remain reusable textures. Preview seeking reconstructs deterministic Pattern state from Show start in the Fast runtime; it does not mutate the saved Show.
