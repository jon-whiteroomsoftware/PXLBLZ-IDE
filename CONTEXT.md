...
The editor-header action — enabled only when a Controller is connected **and** its dimensionality matches the pattern — that pushes the open **pattern** to the Controller, optionally with its **map**, via a **preflight** dialog that surfaces the IDE-only warnings (does the map fit the Controller's fixed pixel count, are its grid dims sane for the hardware, etc.). The minimal push payload is **pattern (always) + map (opt-in, guarded)** — *nothing else from the preview rides along*: not brightness, fit (Fill/Contain), pixel count, speed, light size, diffusion, solidity, fidelity, or layout. Those are explicitly "maybe later." Push is a deliberate user click, not a continuous sync — which also keeps Controller flash writes rare. The map push is guarded because a Controller stores **one shared map** that may be the only one its hardware supports. **One verb, overwrite-in-place:** push both runs *and* stores the pattern; the IDE remembers a per-Controller binding (this IDE pattern ↔ that Controller-assigned id) so re-pushing during the iterate loop **updates the same stored entry** rather than piling up copies — first push creates and binds, later pushes overwrite, a re-create silently re-binds if the entry was deleted on the Controller. Control values are *not* part of the payload either — a pushed pattern comes up with its own control values, tuned live afterward in the **Controller panel**.
_Avoid_: "deploy" (reserved for **Deploy to preview**, the map→preview action); "upload" as the canonical verb (the verb is push / Send to Controller).
