...
**Left rail**:
The app's object browser and primary open/select surface. Its top-level organization follows **object type** before provenance: **Patterns**, **Maps**, **Mixins**, **Controllers**, and **Shows** live as rail modes, with **Catalog** as the bottom entry for built-in resources. **Patterns** lists personal patterns; **Maps** lists personal custom maps; **Controllers** lists durable hardware profiles. The **dimension lens** and type-down name search belong only to dimensional authoring content (Patterns and Maps), not Controllers. Switching rail mode changes what the rail is browsing, not necessarily which file or page remains open in the editor.
