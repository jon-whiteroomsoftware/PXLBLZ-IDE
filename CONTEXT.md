...
**Template** (stock-map):
The verbatim plain-JS **source** of a source-backed **stock map** (Square, Wide 2:1, the Cube/Sphere/Star shell and volume maps, Ring, the helix cloud), loaded into the **New Map** editor as a starting point for a new **custom map**. The **only** way to see stock-map code — stock maps are never openable in place; loading a template forks an editable custom copy. A template load copies **source text only** — never name (user-set) or `dim` (inferred at bake from the returned arity). Browsable from a **"Load template" dropdown** in the New Map editor: loading is silent when the buffer is pristine (byte-identical to the last-loaded baseline — skeleton or a previously-loaded template) and **confirms before overwriting** once edited.
_Avoid_: seed (reserved for the example clouds' historical IDB rows); preset.

**Recommended map** (demo → map association):
A purely **IDE-side**, preview-only pointer from a **demo** to the **stock map** it is meant to be viewed on (e.g. the sphere-rings demo recommends the Sphere shell). It is *not* part of the Pixelblaze universe — the physical **Controller** knows only patterns and maps, never associations — and it lives **nowhere in the pattern source or the transpiled artifact**: it is a small registry in the demo-loading layer (`demoName → mapId`), consumed only by the **Layout** resolver as the default when a pattern has no persisted layout selection. The same category as the existing per-pattern layout selection (preview-only, never sent to hardware), just table-sourced. The map stays freely switchable; the recommendation only sets the on-open default. Applies to **demos** (read-only, no `PatternRecord`); a **custom pattern** persists its own selection instead. A sibling registry of the same category, **recommended pixel count** (`demoName → count`), sets the on-open count ahead of the per-dimension default (e.g. the sphere demo opens at 4096, not the 3D cube's 512) — a demo carries no persisted count, and a sparse cloud reads poorly; the count box stays freely editable. A third sibling, **recommended solidity** (`demoName → number`, ADR-0011), sets a demo's on-open **solidity** ahead of the default `1.0` (a glass-ornament demo at `0.3`); the slider stays freely editable.
