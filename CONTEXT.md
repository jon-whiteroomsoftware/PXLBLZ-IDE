...
- the **Map** control — owns `sample`; always present for 2D/3D. For 1D it appears only when the workspace contains a true `[x]` map, and includes **Index** as the reversible no-installed-map default (`x = index / pixelCount`).
...
So a **1D** pattern always shows shape and conditionally shows map, a **2D** pattern shows two (map + surface, map on the left), and a **3D** pattern shows one (map). The earlier single "Layout dropdown" union is **retired** (it conflated `sample` selection with `pos` selection; the §**Sample / position** split now has one control each). "Layout" survives only as this collective noun; elsewhere it stays a forbidden synonym for **map**.
...
Which of **1D / 2D / 3D** a pattern runs as, or a map supplies — always the Pattern/map domain, not the preview embedding's display dimension. Named by the render fn via a bijection: `render` → 1D, `render2D` → 2D, `render3D` → 3D. A `render(index, x)` Pattern is **1D**; `x` comes from a true 1D map or Pixelblaze's implicit normalized-index convention. A Pattern's dimensionality is the highest render fn it defines.
