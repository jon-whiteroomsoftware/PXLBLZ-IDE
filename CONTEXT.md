...
The storage seam behind **Cloud Patterns**, **Cloud Maps**, **Cloud Mixins**, **Cloud Libraries**, and durable **Controller profiles**. Durable personal resources have one supported backend: authenticated Cloudflare Pages Functions backed by D1. Signed-out users run in non-durable **demo mode**, where built-in demos, stock maps, stock mixins, libraries, docs, and preview controls remain usable but personal create/update/delete actions require sign-in. The remote provider stores personal patterns, custom maps, cloud mixins, cloud libraries, controller profiles, last-active state, demo overrides, and controller push metadata through authenticated Pages Functions. No browser-local-to-D1 migration is attempted. The Cloudflare D1 binding (`PXLBLZ_DB`) and GitHub OAuth/session foundation sit behind this seam so React components do not know about D1, Workers, or auth.
