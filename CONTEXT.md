...
A generated physical point set catalogued once, with one or more honest map-coordinate views over the same LED indexes. The geometry family owns preview `pos`; the selected coordinate view owns hardware-real `sample` and materializes as an ordinary self-contained Pixelblaze map source. **Cylinder** is the first shipped family: **Strand** `[x]` is ordered wire progress, **Surface** `[u,v]` is circumference × height and is the natural default, and **Spatial** `[x,y,z]` is the normalized cylinder wall. Switching views never moves the preview points. A family is not a volume/shell toggle: Cylinder is a wall surface, and a future filled Cylinder volume would be a separate generator.
